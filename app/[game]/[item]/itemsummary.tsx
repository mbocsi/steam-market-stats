  ...
  return (
    <div className="flex flex-col p-4">
      <div className="p-4 flex flex-row justify-between">
        <p>PREVIOUS CLOSE</p>
        <p>{close ? "$" + close[1].toFixed(2).toString() : "$xx.xx"}</p>
      </div>
      <div className="p-4 border-t-2 flex flex-row justify-between">
        <p>DAY RANGE</p>
        <p>
          {day_min ? "$" + day_min[1].toFixed(2).toString() : "$xx.xx"} -{" "}
          {day_max ? "$" + day_max[1].toFixed(2).toString() : "$xx.xx"}
        </p>
      </div>
      <div className="p-4 border-t-2 flex flex-row justify-between">
        <p>54 WEEK RANGE</p>
        <p>
          {year_min ? "$" + year_min[1].toFixed(2).toString() : "$xx.xx"} -{" "}
          {year_max ? "$" + year_max[1].toFixed(2).toString() : "$xx.xx"}
        </p>
      </div>
      <div className="p-4 border-t-2 flex flex-row justify-between">
        <p>MARKET CAP</p>
